...
import { 
  ...
  analyzePromptService, 
  refinePromptService, 
  optimizePromptService,
  generateMagicStoryService,
  generateStoryVariationsService,
  type StoryExpansion
...
export default function Home() {
  ...
  const [generatedStory, setGeneratedStory] = useState<StoryExpansion | null>(null);
  const [storyVariations, setStoryVariations] = useState<string[]>([]);
  ...
  const generateLivePrompt = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await generatePromptService({
        idea,
        model,
        visualStyle,
        cameraMovement,
  ...
  const handleRefineClick = async () => {
    ...
    setAiLoading(true);
    try {
      const refinedPrompt = await refinePromptService(assembledPrompt, model);
      setAssembledPrompt(refinedPrompt);

      // Update history with refined prompt
      setPromptHistory((prevHistory) => {
        const updatedHistory = prevHistory.map((entry) =>
          entry.assembledPrompt === assembledPrompt
            ? { ...entry, assembledPrompt: refinedPrompt, timestamp: Date.now() }
            : entry
        );
        if (typeof window !== 'undefined') {
          localStorage.setItem('promptHistory', JSON.stringify(updatedHistory));
...
`;
  ...
  const analyzePrompt = useCallback(async (prompt: string) => {
    ...
    try {
      const data = await analyzePromptService(prompt);
  ...
  const optimizePrompt = useCallback(async () => {
    ...
    setOptimizing(true);
    try {
      const data = await optimizePromptService(idea, model, assembledPrompt);
      if (data.optimizedPrompt) {
        setAssembledPrompt(data.optimizedPrompt);
        setPromptAnalysis({ score: data.qualityScore || 75, feedback: data.improvements || [] });
      }
    } catch (error) {
  ...
  const generateMagicStory = useCallback(async () => {
    if (!magicWord || magicWord.length < 2) return;

    setMagicLoading(true);
    try {
      console.log('Magic Mode: Starting generation for word:', magicWord, 'model:', model);
      const { story, prompt } = await generateMagicStoryService(magicWord, model);
      console.log('Magic Mode: Generated story:', story);
      console.log('Magic Mode: Generated prompt:', prompt);
      
      setGeneratedStory(story);
      setAssembledPrompt(prompt.assembledPrompt);
      setIdea(story.fullStory);
      setShowStoryPreview(true);

      // Auto-populate the style controls with AI-generated values
      setVisualStyle(story.visualStyle);
      setCameraMovement(story.cameraMovement);
      setBackground(story.background);
      setLightingMood(story.lightingMood);
      setAudioCues(story.audioCues);
      setColorPalette(story.colorPalette);

      // Generate story variations
      const variations = await generateStoryVariationsService(story.fullStory, model);
      setStoryVariations(variations);

      // Add to history
      const newPromptEntry: GeneratedPromptData = {
        id: Date.now().toString(),
        idea: story.fullStory,
        model,
      ...
      setPromptHistory((prevHistory) => {
        const updatedHistory = [newPromptEntry, ...prevHistory.slice(0, 9)];
        if (typeof window !== 'undefined') {
          localStorage.setItem('promptHistory', JSON.stringify(updatedHistory));
        }
        return updatedHistory;
      ...
      console.error('Magic Mode: Failed to generate story:', error);
      alert(`Magic Mode Error: ${error.message || 'Failed to generate story. Please try again.'}`);
    } finally {
  ...
  const regenerateStory = useCallback(async () => {
    if (!magicWord) return;
    await generateMagicStory();
  }, [magicWord, generateMagicStory]);

  const selectStoryVariation = useCallback(async (variation: string) => {
    setIdea(variation);
    setGeneratedStory(prev => prev ? { ...prev, fullStory: variation } : null);
    
    // Regenerate prompt with new story
    if (generatedStory) {
      const prompt = await generatePromptService({
        idea: variation,
        model,
        visualStyle: generatedStory.visualStyle,
        cameraMovement: generatedStory.cameraMovement,
        background: generatedStory.background,
        lightingMood: generatedStory.lightingMood,
        audioCues: generatedStory.audioCues,
        colorPalette: generatedStory.colorPalette,
        negativePrompts: model === 'veo3' ? 'blurry, low-quality, subtitles, text overlay' : 'blurry, low-quality, cartoonish'
      });
      setAssembledPrompt(prompt.assembledPrompt);
    }
  }, [generatedStory, model]);
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-6 pb-20">
          ...
          <div className="flex justify-center">
            ...
            >
              <div className="flex items-center space-x-6">
                <span className="text-lg font-cyber text-white">Mode:</span>
                
                <motion.button
                  onClick={() => setMagicMode(false)}
                  className={`px-6 py-3 rounded-2xl font-cyber font-bold transition-all duration-300 ${
                    !magicMode
                      ? 'bg-neon-gradient text-black shadow-lg shadow-neon-green/50'
                ...
                <motion.button
                  ...
                  className={`px-6 py-3 rounded-2xl font-cyber font-bold transition-all duration-300 ${
                    magicMode
                      ? 'bg-aurora text-black shadow-lg shadow-neon-purple/50'
                      : 'bg-white/10 text-white hover:bg-white/20'
                  }`}
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
              ...
              >
                {magicMode 
                  ? "Type just one word and AI creates a complete 8-second cinematic story!"
                  : "Full control over every aspect of your video prompt generation."
                }
              </motion.p>
            </motion.div>
          </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8 mb-16">
          ...
          <motion.div
            ...
            className="space-y-8"
          >
            {/* Magic Mode / Advanced Mode Input */}
            {magicMode ? (
              /* Magic Mode - One Word Input */
              <div className="bg-white/5 backdrop-blur-2xl border border-white/10 rounded-3xl p-8 shadow-2xl hover:shadow-aurora/20 transition-all duration-500">
                <div className="flex items-center space-x-3 mb-6">
                  <div className="w-8 h-8 bg-aurora rounded-full flex items-center justify-center animate-pulse">
                    <Wand className="w-5 h-5 text-black" />
                  </div>
                  <h3 className="text-2xl font-cyber font-bold text-white">✨ Magic Word</h3>
                </div>
                ...
                <div className="relative">
                  <motion.input
                    ...
                    type="text"
                    value={magicWord}
                    onChange={(e) => setMagicWord(e.target.value.replace(/\s+/g, ''))} // Only allow single words
                    placeholder="Enter just one word... (e.g., cat, sunset, ocean)"
                    className="w-full p-6 bg-white/10 border border-white/20 rounded-2xl text-white placeholder-gray-400 text-xl text-center focus:outline-none focus:border-aurora focus:ring-2 focus:ring-aurora/50 transition-all duration-300"
                  ...
                  <motion.button
                    ...
                    disabled={magicLoading || !magicWord || magicWord.length < 2}
                    className="w-full mt-4 py-4 px-8 bg-aurora hover:bg-neon-gradient text-black font-cyber font-bold text-lg rounded-2xl shadow-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300"
                  ...
                  <AnimatePresence>
                    {showStoryPreview && generatedStory && (
                      <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: -20 }}
                        className="mt-6 p-6 bg-black/20 border border-white/10 rounded-2xl"
                      >
                        <div className="flex items-center justify-between mb-4">
                          <h4 className="text-lg font-cyber font-bold text-white flex items-center">
                            <Eye className="w-5 h-5 mr-2 text-aurora" />
                            Story Preview
                          </h4>
                          <motion.button
                        ...
                        {storyVariations.length > 0 && (
                          <div className="space-y-2">
                            <h5 className="text-sm font-semibold text-white flex items-center">
                              <Shuffle className="w-4 h-4 mr-2 text-neon-green" />
                              Try These Variations:
                            </h5>
                            {storyVariations.map((variation, index) => (
                              <motion.button
                                key={index}
                                onClick={() => selectStoryVariation(variation)}
                                className="w-full p-3 text-left bg-white/5 hover:bg-white/10 rounded-xl text-gray-400 hover:text-white transition-all duration-300 text-sm"
                                whileHover={{ x: 5 }}
                              >
                                <Sparkles className="w-3 h-3 inline mr-2 text-neon-blue" />
                                {variation}
                              </motion.button>
                            ))}
                          </div>
              ...
              /* Advanced Mode - Traditional Input */
            ...
            <div className="bg-white/5 backdrop-blur-2xl border border-white/10 rounded-3xl p-8 shadow-2xl">
              ...
              <div className="grid grid-cols-2 gap-4">
                {['veo3', 'flow', 'runway', 'pika'].map((m) => (
                  <motion.button
                    key={m}
                    onClick={() => setModel(m as 'veo3' | 'flow' | 'runway' | 'pika')}
            ...
            {/* Action Buttons - Only show in Advanced Mode */}
            {!magicMode && (
